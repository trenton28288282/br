...
function performLogin(id){
  ...
  // Optimize runtime-created images: ensure button images and result image use lazy loading and have explicit dimensions
  try {
    const btnImgs = document.querySelectorAll('.btn-img');
    btnImgs.forEach(img => {
      // prefer lazy loading for decorative button backgrounds
      try { img.loading = 'lazy'; } catch(e){}
      // set sensible fallback dimensions so layout doesn't shift
      if (!img.hasAttribute('width')) img.setAttribute('width', '360');
      if (!img.hasAttribute('height')) img.setAttribute('height', '120');
    });

    const resultImg = document.getElementById('result-image');
    if (resultImg){
      try { resultImg.loading = 'lazy'; } catch(e){}
      if (!resultImg.hasAttribute('width')) resultImg.setAttribute('width','220');
      if (!resultImg.hasAttribute('height')) resultImg.setAttribute('height','220');
    }
  } catch (e){}
